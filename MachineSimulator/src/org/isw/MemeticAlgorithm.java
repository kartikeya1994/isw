...
public class MemeticAlgorithm {
	...
	private Chromosome[] crossover(Chromosome parent1, Chromosome parent2) {
		...
		int crossoverPoint = rand.nextInt(Machine.compList.length-1) + 1;
	...
	private void evaluateFitness(ArrayList<Chromosome> list) throws InterruptedException, ExecutionException {
		...
		for(Chromosome chromosome : list){
			...
			pool.submit(new SimulationThread(schedule,chromosome.combo,pmOpportunity));
			cnt++;
		}
		for(int i=0;i<cnt;i++){
			SimulationResult result = pool.take().get();
			map.get(result.compCombo).fitnessValue = result.cost;
	...
	private void initializePopulation() {
		...
		for(int i=0;i<populationSize;i++){
			  population.add(new Chromosome(rand.nextInt((int)Math.pow(2, Machine.compList.length)) + 1));
		}
